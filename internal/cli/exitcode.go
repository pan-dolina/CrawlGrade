...
const (
	ExitOK         = 0 // audit completed, no finding reached --fail-on
	ExitFindings   = 1 // at least one finding reached the --fail-on threshold
	ExitUsage      = 2 // invalid arguments or flags
	ExitBlocked    = 3 // the target is not auditable: blocked by the network policy or robots.txt
	ExitIncomplete = 4 // the start URL could not be fetched, or the run was interrupted
