...
pkg="github.com/pan-dolina/crawlgrade/internal/version"
ldflags="-s -w -buildid= -X ${pkg}.Version=${VERSION} -X ${pkg}.Commit=${COMMIT} -X ${pkg}.Date=${DATE}"

if [ -d "$DIST" ] && [ -n "$(ls -A "$DIST")" ]; then
  echo "output directory must be empty: $DIST" >&2; exit 1
fi
mkdir -p "$DIST"
work="$(mktemp -d)"
...
for target in $TARGETS; do
  goos="${target%/*}"
  goarch="${target#*/}"
  name="crawlgrade_${VERSION#v}_${goos}_${goarch}"
  exe="crawlgrade"
  [ "$goos" = "windows" ] && exe="crawlgrade.exe"

  echo "building $name"
  mkdir -p "$work/$name"
  CGO_ENABLED=0 GOOS="$goos" GOARCH="$goarch" GOFLAGS="${GOFLAGS:+$GOFLAGS }-mod=readonly" \
    go build -trimpath -buildvcs=true -ldflags "$ldflags" -o "$work/$name/$exe" ./cmd/crawlgrade
