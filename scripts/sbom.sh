...
if [ "$#" -eq 0 ]; then
  ...
  cp go.mod go.sum "$source_dir/"
  syft scan "dir:$source_dir" --source-name crawlgrade --source-version "${VERSION:-unknown}" \
    --override-default-catalogers go-module-file-cataloger --select-catalogers -file \
    -o "spdx-json=$out/crawlgrade.spdx.json" \
    -o "cyclonedx-json=$out/crawlgrade.cdx.json"
  exit 0
fi
